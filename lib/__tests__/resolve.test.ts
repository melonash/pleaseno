...
describe("resolveTurn", () => {
  ...
  it("wins when a top-level, plausible attempt pushes the meter over the threshold", () => {
    ...
    expect(r.delta).toBe(54);
    expect(r.state.meter).toBe(64);
  ...
  it("applies the offence penalty and reads as hostile", () => {
    ...
    expect(r.delta).toBe(-14 - TUNING.OFFENCE_PENALTY);
  ...
  it("dampens a repeated approach", () => {
    ...
    expect(fresh.delta).toBe(31);
    expect(repeat.delta).toBe(Math.round(31 * TUNING.REPEAT_APPROACH_FACTOR));
  });
