...
export function buildQuestions(scene: Scene, freeKind: FreeKind = "cool") {
  return {
    ...
    contradicts_situation: noul(
      "Does `current_attempt.text` assert something that flatly contradicts the established situation in `scene.situation` or the player's own earlier messages in `conversation_so_far`? Ordinary lies, excuses and exaggerations are allowed and are NOT contradictions. A contradiction is claiming to be someone the situation rules out (the pilot, the police chief, the NPC's boss or relative), asserting a yes the NPC never gave (\"you already said I could board\"), or reversing a fact the player already stated. Thanking the NPC, reacting to or accurately repeating what the NPC just said in `conversation_so_far`, or hoping aloud that they will help is ordinary conversation, NOT a contradiction, even if it sounds like the player assumes a yes. Changing their mind, backing down, or giving in ('fine, I'll come') is also NOT a contradiction; only facts can be contradicted, not wishes.",
      {
